    ...
    let footer = match state.phase {
        MetadataEditorPhase::Editing => {
            ...
            if state.any_presentation_dirty() {
                spans.push(footer_pill("Alt+A Apply", theme.green, theme));
                spans.push(pill_gap());
            }
            spans.push(footer_pill("Alt+O OK", theme.green, theme));
            spans.push(pill_gap());
...
fn draw_metadata_read_only_footer(
    ...
    if state.any_presentation_dirty() {
        pills.push(("Alt+A Apply", theme.green, None));
    }
    pills.push(("Alt+O OK", theme.green, None));
