...
///
/// Mirrors `super::musicbrainz::populate_editor_from_mb` for single-
/// image rips: when the editor is opened on one file representing a
/// multi-track release, populate per-track TITLE values into per-
/// track entries. ARTIST broadcasts across track-dim slots since
/// gnudb has no per-track artist data. Album-level fields (ALBUM /
/// DATE / GENRE) and TRACKNUMBER stay at file dimension.
///
/// Eligibility for per-track populate is stricter than MB: gnudb has
/// no identity tag and no length verification. We require an
/// existing CUESHEET in `state.entries` (sidecar-injected by Phase 2
/// or pre-existing embedded) so Phase 4 has a structural anchor for
/// save-time regen. Without an anchor we'd create per-track entries
/// the user couldn't persist.
pub fn populate_editor_from_gnudb(
    state: &mut super::app::MetadataEditorState,
    entry: &GnudbEntry,
) {
    let n = state.paths.len();
    let single_image = n == 1 && entry.tracks.len() > 1;
    ...
    let track_dim = if per_track_populate { entry.tracks.len() } else { n };
    ...
    // Pre-existing TITLE/ARTIST entries (Phase 2 sidecar-grown or
    // simply read from file tags) get resized to gnudb's track count.
    ...
    if per_track_populate {
        // Per-track populate: TITLE per slot from gnudb.tracks[i];
        // ARTIST broadcasts (gnudb has one artist for whole disc);
        // album-level fields write to slot 0.
        ...
        state.entries[album_idx].per_file_values[0] = entry.album.clone();
        state.entries[tn_idx].per_file_values[0] = "1".to_string();
        if !entry.year.is_empty() {
            state.entries[year_idx].per_file_values[0] = entry.year.clone();
        }
        if !entry.genre.is_empty() {
            state.entries[genre_idx].per_file_values[0] = entry.genre.clone();
        }
    } else if single_image {
        // Single-image without a CUESHEET anchor: album-level
        // fallback. Mirrors MB's else-if-single_image branch — write
        // album-level values to dim-1 entries; SKIP entries already
        // grown to per-track dim by some earlier path (defensive
        // even though has_cuesheet=false here).
        ...
        if let Some(idx) = title_dim_one {
            state.entries[idx].per_file_values[0] = entry.album.clone();
        }
        if let Some(idx) = artist_dim_one {
            state.entries[idx].per_file_values[0] = entry.artist.clone();
        }
        state.entries[album_idx].per_file_values[0] = entry.album.clone();
        state.entries[tn_idx].per_file_values[0] = "1".to_string();
        if !entry.year.is_empty() {
        ...
        // Per-file populate (multi-file selection).
        for i in 0..n {
            if let Some(title) = entry.tracks.get(i) {
                state.entries[title_idx].per_file_values[i] = title.clone();
            }
...
/// gets an album-level fallback (mirrors `populate_editor_from_gnudb`).
...
mod gnudb_per_track_tests {
    ...
    #[test]
    fn gnudb_per_track_populates_when_single_image_with_cuesheet() {
        let (mut state, _td) = empty_state(1);
        // Phase 2 would have injected this from a sidecar at editor open.
        state.entries.push(cuesheet_entry(
            "FILE \"a.flac\" FLAC\n  TRACK 01 AUDIO\nINDEX 01 00:00:00\n  TRACK 02 AUDIO\nINDEX 01 00:01:00\n  TRACK 03 AUDIO\nINDEX 01 00:02:00\n",
        ));
        let entry = entry("Disc Album", "1977", "Rock", vec!["T1", "T2", "T3"]);
        populate_editor_from_gnudb(&mut state, &entry);
        ...
        assert_eq!(lookup("TITLE"), vec!["T1", "T2", "T3"], "per-track titles");
        assert_eq!(lookup("ARTIST"),
            vec!["Album Artist", "Album Artist", "Album Artist"],
            "ARTIST broadcasts (gnudb has one artist for whole disc)");
        assert_eq!(lookup("ALBUM"), vec!["Disc Album"], "album-level dim 1");
        assert_eq!(lookup("DATE"), vec!["1977"]);
        assert_eq!(lookup("GENRE"), vec!["Rock"]);
        assert_eq!(lookup("TRACKNUMBER"), vec!["1"], "TRACKNUMBER stays dim 1 on single-image");
    }

    #[test]
    fn gnudb_album_fallback_when_single_image_without_cuesheet() {
        // No CUESHEET anchor → fall back to album-level rather than
        // creating per-track entries Phase 4 can't persist.
        let (mut state, _td) = empty_state(1);
        let entry = entry("Disc Album", "1977", "Rock", vec!["T1", "T2", "T3"]);
        populate_editor_from_gnudb(&mut state, &entry);
        ...
        assert_eq!(lookup("TITLE"), vec!["Disc Album"],
            "no CUESHEET anchor → TITLE = album name (album-level fallback)");
        assert_eq!(lookup("ARTIST"), vec!["Album Artist"]);
        assert_eq!(lookup("ALBUM"), vec!["Disc Album"]);
    }

    #[test]
    fn gnudb_multi_file_keeps_per_file_behavior() {
        // 3 files, 3 tracks → multi-file, per-file index population.
        let (mut state, _td) = empty_state(3);
        let entry = entry("Album", "", "", vec!["T1", "T2", "T3"]);
        populate_editor_from_gnudb(&mut state, &entry);

        let title = state.entries.iter().find(|e| e.display_key == "TITLE").unwrap();
        assert_eq!(title.per_file_values, vec!["T1", "T2", "T3"]);
        let artist = state.entries.iter().find(|e| e.display_key == "ARTIST").unwrap();
    ...
    fn gnudb_per_track_grows_existing_entries_via_ensure_dim_replicate() {
        ...
        // read from file's tags). With CUESHEET present (sidecar inject)
        // and per-track gnudb populate, existing entries get resized
        // and overwritten with gnudb's per-track data.
        let (mut state, _td) = empty_state(1);
        state.entries.push(TagEntry {
            display_key: "TITLE".to_string(),
            item_key: lofty::tag::ItemKey::TrackTitle,
            value: "Whole Album".to_string(),
            original: "Whole Album".to_string(),
            is_binary: false, is_mixed: false,
            per_file_values: vec!["Whole Album".to_string()],
            per_file_originals: vec!["Whole Album".to_string()],
            mb_proposed_value: None, mb_proposed_per_file: None,
        });
        state.entries.push(cuesheet_entry(
            "FILE \"a.flac\" FLAC\n  TRACK 01 AUDIO\nINDEX 01 00:00:00\n  TRACK 02 AUDIO\nINDEX 01 00:01:00\n",
        ));
        let entry = entry("Disc", "", "", vec!["G1", "G2"]);
        populate_editor_from_gnudb(&mut state, &entry);

        let title = state.entries.iter().find(|e| e.display_key == "TITLE").unwrap();
        assert_eq!(title.per_file_values, vec!["G1", "G2"], "values overwritten with gnudb");
        // ensure_dim_replicate replicates "Whole Album" to fill the new dim.
        assert_eq!(title.per_file_originals, vec!["Whole Album", "Whole Album"],
            "originals replicated → revert restores file's pre-populate state");
    }
}
