...
use crate::convert::split_cue_album::{
    ...
    resolve_split_cue_file_reference, select_split_cue_folder_members_with_preference,
    SplitCueAdmissionMember, SplitCueAlbumGroupingDecision, SplitCueAlbumGroupingReason,
    SplitCueFolderPreference, SplitCueFolderSelection, SplitCueMemberRejection,
...
/// Persistable per-folder CUE policy. `AutomaticWholeAlbum` is an explicit
/// opt-in to assemble all useful viable, pairwise-disjoint members even when
/// their FILE references needed a safe extension/case fallback. A `Cue` choice
/// selects exactly one viable representation while it remains viable. If that
/// saved CUE later becomes non-viable, ordinary folder policy resumes so the
/// stale override cannot manufacture ambiguity; `AutomaticWholeAlbum` remains
/// strict because an unavailable automatic policy reflects current overlap.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum QueueCueSelectionOverride {
    AutomaticWholeAlbum,
    Cue(PathBuf),
}

/// Operation/session-scoped folder -> CUE policy. Keys and embedded paths are
/// canonicalized by the consumer, so callers may retain user-facing paths.
pub type QueueCueSelectionOverrides = BTreeMap<PathBuf, QueueCueSelectionOverride>;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum QueueCueSelectionVerdict {
    Automatic,
    Viable,
    Rejected,
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct QueueCueSelectionRow {
    pub label: String,
...
pub struct QueueCueSelectionPrompt {
    pub parent: PathBuf,
    /// Legacy best-ranked CUE candidates retained for non-interactive
    /// consumers that inspect candidate metadata. Interactive choice UIs must
    /// use `rows`, which intentionally carries the wider viable/rejected view
    /// plus automatic policy.
    pub candidates: Vec<PathBuf>,
    pub rows: Vec<QueueCueSelectionRow>,
}
...
impl QueueCueSelectionPrompt {
    ...
    pub fn from_members(
        ...
        let candidates = candidates
            ...
            .collect::<Vec<_>>();
        let has_viable = !viable.is_empty();
        let mut rows = Vec::with_capacity(1 + viable.len() + rejected.len());
        rows.push(QueueCueSelectionRow {
            label: "Let tonepoet handle it".to_string(),
            verdict: QueueCueSelectionVerdict::Automatic,
            selection: automatic_available.then_some(QueueCueSelectionOverride::AutomaticWholeAlbum),
            reason: (!automatic_available).then(|| {
                if has_viable {
                    "automatic whole-album assembly is unavailable because the viable CUEs overlap"
                        .to_string()
                } else {
                    "automatic whole-album assembly is unavailable because no viable CUEs remain"
                        .to_string()
                }
            }),
            repair_cue: None,
        ...
        rows.extend(viable.into_iter().map(|member| QueueCueSelectionRow {
            label: member
                .cue_path
                .file_name()
                .and_then(|name| name.to_str())
                .map(str::to_owned)
                .unwrap_or_else(|| member.cue_path.display().to_string()),
    ...
    #[must_use]
    pub fn first_selectable_row(&self) -> usize {
        self.rows.iter().position(QueueCueSelectionRow::selectable).unwrap_or(0)
    }
    ...
    pub fn noninteractive_error_message(&self) -> String {
        let viable = self
            .rows
            .iter()
            .filter(|row| row.verdict == QueueCueSelectionVerdict::Viable)
            .map(|row| row.label.as_str())
            .collect::<Vec<_>>()
            .join(", ");
        format!(
            "multiple viable CUE files in {} require a selection; choose one explicitly or opt into automatic whole-album assembly: {}",
            self.parent.display(),
            viable
        )
    }
...
const CUMULATIVE_CUE_DROP_WARNING_PREFIX: &str =
    "Ignored malformed cumulative multi-FILE CUE ";

/// Return the first quiet-auto warning that reports a structurally cumulative
/// multi-FILE CUE being dropped. Callers may continue prioritizing the first
/// ordinary expansion warning while additionally preserving this product-
/// required diagnostic when it appears later in the warning vector.
#[must_use]
pub(crate) fn first_cumulative_cue_drop_warning(expansion_errors: &[String]) -> Option<&str> {
    expansion_errors
        .iter()
        .find(|warning| warning.starts_with(CUMULATIVE_CUE_DROP_WARNING_PREFIX))
        .map(String::as_str)
}
...
pub fn expand_paths_to_audio_with_metadata_using_grouping_decisions_and_metadata_priority(
    paths: &[PathBuf],
    grouping_decisions: &QueueSplitCueAlbumGroupingDecisions,
    metadata_target_priority: &[crate::config::AggregateMetadataTarget],
) -> QueueExpansionResult {
    expand_paths_to_audio_with_metadata_using_grouping_decisions_and_cue_selections_and_metadata_priority(
        paths,
        grouping_decisions,
...
pub fn expand_paths_to_audio_with_metadata_using_grouping_decisions_and_cue_selections_and_metadata_priority(
    paths: &[PathBuf],
    grouping_decisions: &QueueSplitCueAlbumGroupingDecisions,
    cue_selections: &QueueCueSelectionOverrides,
    metadata_target_priority: &[crate::config::AggregateMetadataTarget],
...
where
    F: FnMut() -> bool,
{
    expand_paths_to_audio_with_preserved_disc_roots_limited_using_grouping_decisions_and_cue_selections_and_metadata_priority(
        paths,
        preserved_disc_roots,
        max_visited,
        is_cancelled,
...
pub fn expand_paths_to_audio_with_preserved_disc_roots_limited_using_grouping_decisions_and_cue_selections_and_metadata_priority<F>(
    paths: &[PathBuf],
    preserved_disc_roots: &[PathBuf],
    max_visited: usize,
    mut is_cancelled: F,
        ...
        for cue in cue_sheets {
            ...
            match decision {
                Ok(CueQueueDecision::SplitSource { referenced_audio }) => {
                    // A discovered single-image CUE is one metadata
                    // representation of the carrier. If File tags win the
                    // configured aggregate hierarchy, retain the raw carrier
                    // in the queue instead of suppressing it in favor of CUE
                    // track structure. Explicit .cue selection remains an
                    // exact source operation and bypasses this aggregate rung.
                    let file_tags_authoritative = !cue.explicit
                        && cue.admitted_member.as_ref().is_some_and(|member| {
                            if member.referenced_audio.len() != 1 {
                                return false;
                            }
                            let audio = &member.referenced_audio[0];
                            let individual_files = crate::convert::pipeline::materializer_single::individual_file_metadata_source_is_viable(audio);
                            let embedded_cue = crate::convert::pipeline::materializer_cue::embedded_single_image_cuesheet_is_automatic_authority_viable(audio);
    ...
    for (parent, mut candidates) in by_parent {
        ...
        match select_split_cue_folder_members_with_preference(&cue_paths, preference) {
            SplitCueFolderSelection::Selected {
                ...
                let explicit_choice_honored = match selected {
                    None => false,
                    Some(QueueCueSelectionOverride::AutomaticWholeAlbum) => true,
                    Some(QueueCueSelectionOverride::Cue(path)) => {
                        let selected_key = queue_path_key(path);
                        members.iter().any(|member| {
                            queue_path_key(&member.cue_path) == selected_key
                        })
                    }
                };
                ...
                for rejection in &rejected {
                    ...
                    if rejection.reason.is_cross_file_cumulative_index() {
                        warnings.push(if !explicit_choice_honored {
                            format!(
                                "Ignored malformed cumulative multi-FILE CUE {} and preserved the surviving folder CUE set for automatic album assembly: {}. Use CUE choices… from Browse to inspect or override the folder policy.",
                                rejection.cue_path.display(),
                                rejection.reason,
                            )
                        } else {
                            format!(
                                "Ignored malformed cumulative multi-FILE CUE {} while honoring the folder's explicit CUE choice: {}.",
                                rejection.cue_path.display(),
                                rejection.reason,
                            )
                        });
                ...
                let materialize_selected_multi_file_as_synthetic = !explicit_choice_honored
                    && members.len() == 1
            ...
            SplitCueFolderSelection::NeedsChoice {
                ...
                return QueueCueCandidateSelection::NeedsChoice(
                    QueueCueSelectionPrompt::from_members(
                        parent,
                        candidates,
                        viable,
                        rejected,
            ...
            SplitCueFolderSelection::NoViable { rejected } => {
                ...
                let detail = rejected
                    ...
                    .join("; ");
                warnings.push(format!(
                    "No viable CUE remains in {} after structural/local checks; using ordinary audio-file discovery{}",
                    parent.display(),
                    if detail.is_empty() {
                        String::new()
                    } else {
...
fn authoritative_failed_merged_groups(
    ...
    for decision in grouping_decisions.values() {
        for group in &decision.groups {
            ...
            // A structurally cumulative combined CUE is a broken redundant
            // representation when every image it names is still fully covered
            // by surviving admitted members. Do not reinterpret that rejected
            // superset as a failed album *member* and suppress the good per-disc
            // set; other rejection classes retain the established fail-closed
            // behavior.
            let selected_audio_keys: HashSet<PathBuf> = selected_members
                .iter()
                .filter(|member| group_keys.contains(&queue_path_key(&member.cue_path)))
                .flat_map(|member| member.referenced_audio.iter())
                .map(|path| queue_path_key(path))
                .collect();
            let redundant_cumulative_alternatives = !validated.rejections.is_empty()
                && validated.rejections.iter().all(|rejection| {
                    rejection.reason.is_cross_file_cumulative_index()
                        && !rejection.resolved_in_folder_audio.is_empty()
                        && rejection
                            .resolved_in_folder_audio
                            .iter()
                            .all(|path| selected_audio_keys.contains(&queue_path_key(path)))
                });
            if redundant_cumulative_alternatives {
                continue;
            }
...
fn push_synthetic_cue_album_groups_for_queue(
    ...
    for (parent, mut candidates) in by_parent {
        ...
        for group in decision.groups {
            ...
            let text = match generate_queue_synthetic_cue_album_with_metadata_priority(
                &group_parts,
                metadata_target_priority,
            ) {
                Ok(text) => text,
...
fn coherent_embedded_cuesheet_for_member_audio_with_base_dirs(
    ...
    for base_dir in base_dirs {
        if let Some(text) = coherent_embedded_cuesheet_for_member_audio(member_audio, base_dir) {
            return Some(text);
        }
    }
    None
...
fn discover_native_multi_file_embedded_groups_for_queue(
    ...
    for path in queueable_non_cue {
        let key = queue_path_key(path);
        if explicit_non_cue_keys.contains(&key) || !is_audio_file_path(path) {
            continue;
        }
...
fn generate_queue_synthetic_cue_album_with_metadata_priority(
    ...
    let embedded_text = coherent_embedded_cuesheet_for_member_audio_with_base_dirs(
        &member_audio,
        &base_dirs,
    );
...
pub fn cue_artifact_commit_decision_for_path(
    ...
    // A queued user-visible CUE is itself a sidecar representation. For a
    // single-image CUE whose image also carries a valid embedded CUESHEET,
    // resolve those peers from the same configured authority selector used by
    // the editor. IndividualFiles does not participate here: queuing the CUE
    // explicitly or as an admitted image source already selected CUE-shaped
    // logical-track structure.
    if is_cue_sheet_path(path) {
        ...
        let embedded_cue = matches!(
            cue_source_policy,
            CueSidecarPolicy::PreferEmbedded
                | CueSidecarPolicy::PreferSidecar
                | CueSidecarPolicy::EmbeddedOnly
        ) && crate::convert::pipeline::materializer_cue::embedded_cuesheet_is_viable_for_sidecar_cue(path);
        let selected = crate::metadata_authority::resolve_aggregate_metadata_target(
            metadata_target_priority,
            crate::metadata_authority::AggregateMetadataAvailability {
                individual_files: false,
        ...
        let cue_sidecar_override = match selected {
            Some(crate::config::AggregateMetadataTarget::SidecarCue) => {
            ...
            Some(crate::config::AggregateMetadataTarget::IndividualFiles) => {
                unreachable!("visible CUE commit excludes IndividualFiles from viability")
            }
            None => Some(cue_source_policy),
        };
        return CueArtifactCommitDecision {
    ...
    let is_cue_artifact_audio = cue_artifact_audio.contains(path)
        || cue_artifact_audio
            .iter()
            .any(|candidate| same_queue_identity(candidate, path));
    if !is_cue_artifact_audio {
        if cue_source_policy == CueSidecarPolicy::IgnoreCue {
            return CueArtifactCommitDecision {
                cue_sidecar_override: Some(CueSidecarPolicy::IgnoreCue),
                sidecar_cue_track_metadata: None,
        ...
        // A directly queued audio carrier can still represent a CUE album when
        // it has a viable neighboring sidecar and/or embedded CUESHEET. Resolve
        // those peers here, before source detection, so later materialization
        // consumes an exact authority policy instead of re-applying a private
        // PreferSidecar/PreferEmbedded rule. Ambiguous sidecar discovery returns
        // None and deliberately falls through to the existing source detector,
        // which retains its fail-closed diagnostic behavior.
        if is_audio_file_path(path) {
            let Some(cue_viability) =
                crate::convert::pipeline::materializer_cue::direct_audio_cue_viability(
                    path,
                    cue_source_policy,
                )
            else {
            ...
            // Prefer* is automatic aggregate authority. A neighboring CUE is
            // relevant to a direct audio click only when existing admission has
            // proved single-wave image structure. A one-track same-stem CUE is
            // still a valid explicit sidecar, but it must not turn an
            // independent file click into implicit Sidecar authority.
            if matches!(
                cue_source_policy,
            ...
            if cue_viability.sidecar_cue || cue_viability.embedded_cue {
                // A CUE-proven single-wave image still has an independently
                // inspectable file-tag representation. Image evidence affects
                // content shape, not whether File tags participate in aggregate
                // metadata authority.
                ...
                let selected = crate::metadata_authority::resolve_aggregate_metadata_target(
                    ...
                    crate::metadata_authority::AggregateMetadataAvailability {
                        individual_files,
                        sidecar_cue: cue_viability.sidecar_cue,
                        embedded_cue: cue_viability.embedded_cue,
                    },
                );
                let cue_sidecar_override = match selected {
                    Some(crate::config::AggregateMetadataTarget::IndividualFiles) => {
            ...
            // Queue admission has now established that neither CUE
            // representation exists for the preferred-policy case. Freeze
            // that result as IgnoreCue so source detection does not repeat the
            // same sidecar-directory and embedded-tag probes for every
            // ordinary file later in the pipeline. Restrictive *Only policies
            // keep their historical downstream diagnostic behavior.
            if matches!(
                cue_source_policy,
                CueSidecarPolicy::PreferEmbedded | CueSidecarPolicy::PreferSidecar
            ) {
    ...
    match crate::metadata_authority::resolve_aggregate_metadata_target(
        ...
        Some(crate::config::AggregateMetadataTarget::SidecarCue) => {
            let sidecar_source = metadata_source.expect(
                "SidecarCue authority is viable only with an admitted metadata mapping",
            );
            return CueArtifactCommitDecision {
                cue_sidecar_override: Some(CueSidecarPolicy::IgnoreCue),
...
mod tests {
    ...
    #[test]
    fn cue_prompt_zero_viable_explains_that_no_viable_cues_remain() {
        let prompt = QueueCueSelectionPrompt::from_members(
            PathBuf::from("album"),
            Vec::new(),
            Vec::new(),
            vec![SplitCueMemberRejection {
                cue_path: PathBuf::from("album/broken.cue"),
                reason: crate::convert::split_cue_album::SplitCueMemberRejectionReason::NoTracks,
                parsed_sheet: None,
        ...
        let automatic = prompt.rows.first().expect("automatic row");
        let reason = automatic.reason.as_deref().expect("disabled reason");
        assert!(reason.contains("no viable CUEs remain"), "{reason}");
...
"#,
    ...
    fn folder_expansion_a1_proven_merged_group_fails_closed_after_member_becomes_unparseable() {
        ...
        let selection = crate::convert::split_cue_album::select_split_cue_folder_members(
    ...
    #[test]
    fn cumulative_combined_superset_does_not_fail_closed_a_proven_per_side_album() {
        let td = tempfile::tempdir().expect("tempdir");
        ...
        // Capture authoritative grouping while all three file objects are
        // canonically admissible. Folder policy subsequently rejects only the
        // redundant cumulative representation.
        let proven_paths = vec![cue_a.clone(), cue_b.clone(), combined.clone()];
        let decision = crate::convert::split_cue_album::merge_decision(
            &proven_paths,
            SplitCueAlbumGroupingReason::TitleSharedPrefix,
        )
        ...
        let mut decisions = QueueSplitCueAlbumGroupingDecisions::new();
        decisions.insert(
            split_cue_album_grouping_key_for_queue(&proven_paths),
            decision,
        );
        ...
        assert!(
            !expanded.expansion_errors.iter().any(|error| {
                error.contains("Nothing was staged for this proven group")
                    || error.contains("Cannot queue merged CUE album")
            }),
            "a rejected redundant superset must not fail-close the surviving per-side album: {:?}",
            expanded.expansion_errors,
        );
        assert!(expanded.expansion_errors.iter().any(|warning| {
            warning.contains("Ignored malformed cumulative multi-FILE CUE")
                && warning.contains("combined.cue")
        }));
        assert_eq!(expanded.paths.len(), 1, "per-side cues should auto-assemble");
        assert!(is_synthetic_cue_album_artifact(&expanded.paths[0]));
        assert!(!path_list_contains(&expanded.paths, &combined));
...
"#,
        ...
        selections.insert(
            td.path().to_path_buf(),
            QueueCueSelectionOverride::Cue(cue_a.clone()),
        );
        let expanded =
    ...
    #[test]
    fn only_cumulative_combined_cue_falls_back_to_audio_with_clear_warning() {
        let td = tempfile::tempdir().expect("tempdir");
        let side_a = td.path().join("side-a.flac");
        let side_b = td.path().join("side-b.flac");
        let combined = td.path().join("combined.cue");
        ...
        assert!(!path_list_contains(&expanded.paths, &combined));
        assert!(expanded.expansion_errors.iter().any(|warning| {
            warning.contains("No viable CUE")
                && warning.contains("ordinary audio-file discovery")
                && warning.contains("cumulative timestamps")
        }));
    }

    #[test]
    fn genuine_overlap_ambiguity_keeps_cumulative_alternative_visible_but_rejected() {
        let td = tempfile::tempdir().expect("tempdir");
        let side_a = td.path().join("side-a.flac");
        let side_b = td.path().join("side-b.flac");
        std::fs::write(&side_a, b"audio").expect("side A");
        ...
        for name in ["valid-a.cue", "valid-b.cue"] {
            std::fs::write(
                td.path().join(name),
                concat!(
                    "TITLE \"Album\"\n",
            ...
            .expect("valid combined cue");
        }
        let broken = td.path().join("broken.cue");
        std::fs::write(
            &broken,
            concat!(
                "TITLE \"Album\"\n",
        ...
        .expect("broken combined cue");

        let expanded = expand_paths_to_audio_with_metadata(&[td.path().to_path_buf()]);
        assert!(expanded.paths.is_empty());
        let prompt = expanded
            .cue_selection_prompt
            .expect("two overlapping valid album representations must remain ambiguous");
        assert_eq!(prompt.candidates.len(), 2, "legacy best-ranked ambiguity is preserved");
        let automatic = prompt
            .rows
            .iter()
            .find(|row| row.verdict == QueueCueSelectionVerdict::Automatic)
            .expect("recommended automatic row");
        assert!(automatic.recommended);
        assert!(automatic.selection.is_none(), "overlapping valid albums cannot be auto-joined");
        let rejected = prompt
            .rows
            .iter()
            .find(|row| row.verdict == QueueCueSelectionVerdict::Rejected)
            .expect("broken cumulative alternative stays discoverable");
        assert_eq!(rejected.label, "broken.cue");
        assert!(rejected.selection.is_none());
        assert!(rejected
            .reason
            .as_deref()
            .is_some_and(|reason| reason.contains("cumulative timestamps")));
    }

    #[test]
    fn fallback_per_side_prompt_keeps_malformed_combined_cue_visible_but_unselectable() {
        let td = tempfile::tempdir().expect("tempdir");
        let side_a = td.path().join("side-a.wv");
        let side_b = td.path().join("side-b.wv");
        let cue_a = td.path().join("side-a.cue");
        let cue_b = td.path().join("side-b.cue");
        let combined = td.path().join("combined.cue");
        std::fs::write(&side_a, b"wavpack placeholder").expect("side A");
        std::fs::write(&side_b, b"wavpack placeholder").expect("side B");
        std::fs::write(
            &cue_a,
        ...
        let prompt = expanded
            .cue_selection_prompt
            .expect("non-exact per-side references should keep the chooser visible");
        ...
        assert_eq!(auto.selection, Some(QueueCueSelectionOverride::AutomaticWholeAlbum));
        let rejected = prompt
            .rows
            .iter()
            .find(|row| row.verdict == QueueCueSelectionVerdict::Rejected)
            .expect("rejected combined CUE row");
        ...
        assert!(resolved.cue_selection_prompt.is_none());
        assert_eq!(resolved.paths.len(), 1);
        assert!(is_synthetic_cue_album_artifact(&resolved.paths[0]));
        cleanup_synthetic_cue_artifacts(&resolved.synthetic_cue_artifacts);
    }

    #[test]
    fn stale_persisted_cue_override_falls_back_to_clean_per_side_auto_assembly() {
        let td = tempfile::tempdir().expect("tempdir");
        let side_a = td.path().join("side-a.flac");
        let side_b = td.path().join("side-b.flac");
        let cue_a = td.path().join("side-a.cue");
        ...
        let proven_paths = vec![cue_a.clone(), cue_b.clone(), rejected_cue.clone()];
        let decision = crate::convert::split_cue_album::merge_decision(
            &proven_paths,
            SplitCueAlbumGroupingReason::TitleSharedPrefix,
        )
        ...
        selections.insert(
            td.path().to_path_buf(),
            QueueCueSelectionOverride::Cue(rejected_cue.clone()),
        );
        let expanded =
            expand_paths_to_audio_with_metadata_using_grouping_decisions_and_cue_selections(
                &[td.path().to_path_buf()],
        ...
        assert!(expanded.cue_selection_prompt.is_none());
        assert_eq!(expanded.paths.len(), 1, "surviving per-side cues should auto-assemble");
        assert!(is_synthetic_cue_album_artifact(&expanded.paths[0]));
        assert!(expanded.expansion_errors.iter().any(|warning| {
            warning.contains("Ignored malformed cumulative multi-FILE CUE")
                && warning.contains("combined.cue")
                && warning.contains("preserved the surviving folder CUE set for automatic album assembly")
        }));
        assert!(expanded.expansion_errors.iter().all(|warning| {
            !warning.contains("while honoring the folder's explicit CUE choice")
        }));
        assert!(expanded.expansion_errors.iter().all(|warning| {
            !warning.contains("Nothing was staged for this proven group")
    ...
    fn metadata_sidecar_alternatives_prompt_then_queue_the_selected_image_once() {
        ...
        selections.insert(
            td.path().to_path_buf(),
            QueueCueSelectionOverride::Cue(second.clone()),
        );
        let expanded =
            expand_paths_to_audio_with_metadata_using_grouping_decisions_and_cue_selections(
                &[td.path().to_path_buf()],
    ...
    fn equally_ranked_distinct_fallback_cues_prompt_and_selected_cue_owns_the_operation() {
        ...
        selections.insert(
            td.path().to_path_buf(),
            QueueCueSelectionOverride::Cue(cue_b.clone()),
        );
        let expanded =
            expand_paths_to_audio_with_metadata_using_grouping_decisions_and_cue_selections(
                &[td.path().to_path_buf()],
...
mod planner_embedded_authority_tests {
    ...
    fn synthetic_album_read_only_embedded_obeys_aggregate_metadata_priority() {
        ...
        for (cue_name, original_title) in [
            ("side_a.cue", "The Dark Side Of The Moon Side A"),
            ("side_b.cue", "The Dark Side Of The Moon Side B"),
        ] {
            let cue_path = td.path().join(cue_name);
            let cue_text = std::fs::read_to_string(&cue_path).expect("read split sidecar");
            std::fs::write(
                &cue_path,
                cue_text.replace(original_title, "Sidecar Album"),
            )
            .expect("write sidecar authority sentinel");
        }

        let embedded = authoritative_album_cue("Embedded Album");
        ...
        assert_eq!(
            crate::tui::keybindings::explicit_native_multi_file_embedded_title_for_test(&[
                side_a, side_b,
            ])
    ...
    #[test]
    fn queued_single_image_cue_uses_configured_sidecar_embedded_authority() {
        if !require_flac_fixture_tool(
            "queued_single_image_cue_uses_configured_sidecar_embedded_authority",
        ) {
            return;
        }
        use crate::config::AggregateMetadataTarget::{EmbeddedCue, IndividualFiles, SidecarCue};
    ...
    #[test]
    fn metadata_artifact_authority_rejects_malformed_embedded_cuesheet() {
        if !require_flac_fixture_tool(
            "metadata_artifact_authority_rejects_malformed_embedded_cuesheet",
        ) {
            return;
        }
        use crate::config::AggregateMetadataTarget::{EmbeddedCue, IndividualFiles, SidecarCue};
        ...
        let td = tempfile::tempdir().expect("tempdir");
        let image = td.path().join("track.flac");
        create_flac(&image);
        let cue = td.path().join("album.cue");
        std::fs::write(
            &cue,
        ...
        .expect("sidecar cue");
        set_embedded_cuesheet(&image, "not a valid cue sheet");

        let artifact_audio = [image.clone()].into_iter().collect::<HashSet<_>>();
        let source = transferred_source(&cue, "track.flac");
        let artifact_metadata = [(image.clone(), source.clone())]
            .into_iter()
            .collect::<BTreeMap<_, _>>();
        ...
        assert_eq!(
            decision.cue_sidecar_override,
            Some(CueSidecarPolicy::IgnoreCue),
            "a malformed embedded tag must not freeze EmbeddedOnly",
        );
        assert_eq!(decision.sidecar_cue_track_metadata, Some(source.clone()));
        ...
        request.source.cue_sidecar = decision
            .cue_sidecar_override
            .expect("artifact authority must be frozen");
        request.source.sidecar_cue_track_metadata = decision.sidecar_cue_track_metadata.clone();
        let queue_metadata =
            crate::convert::processor::dispatch_track_metadata_for_authority_matrix_test(
                &request,
                crate::convert::pipeline::SourceKind::SingleFile,
            )
            .expect("selected sidecar authority must remain consumable by conversion");
        assert_eq!(
            queue_metadata.extra.get("album").map(String::as_str),
            Some("Sidecar Album")
        );
    }
    ...
    #[test]
    fn metadata_artifact_valid_embedded_still_wins_embedded_first_priority() {
        if !require_flac_fixture_tool(
            "metadata_artifact_valid_embedded_still_wins_embedded_first_priority",
        ) {
            return;
        }
        use crate::config::AggregateMetadataTarget::{EmbeddedCue, IndividualFiles, SidecarCue};
        ...
        let artifact_metadata = [(
            image.clone(),
            transferred_source(&cue, "track.flac"),
        )]
        .into_iter()
        .collect::<BTreeMap<_, _>>();
        let decision = cue_artifact_commit_decision_for_path(
    ...
    #[test]
    fn directly_queued_audio_uses_configured_sidecar_embedded_authority() {
        if !require_flac_fixture_tool(
            "directly_queued_audio_uses_configured_sidecar_embedded_authority",
        ) {
            return;
        }
        use crate::config::AggregateMetadataTarget::{EmbeddedCue, IndividualFiles, SidecarCue};
        ...
        assert_eq!(
            ...
            Some(CueSidecarPolicy::IgnoreCue),
            "File tags remain a valid representation of a directly selected single-image carrier",
        );
    ...
    #[test]
    fn directly_queued_independent_audio_ignores_neighboring_one_track_sidecar() {
        if !require_flac_fixture_tool(
            "directly_queued_independent_audio_ignores_neighboring_one_track_sidecar",
        ) {
            return;
        }
        use crate::config::AggregateMetadataTarget::{EmbeddedCue, IndividualFiles, SidecarCue};
        ...
        assert_eq!(
            ...
            Some(CueSidecarPolicy::IgnoreCue),
            "a one-track same-stem CUE does not prove that an independent audio-file click is an image-album source selection",
        );
        let mut direct_request =
            crate::convert::pipeline::materializer_cue::test_pipeline_request_for_authority(
                &selected,
            );
        direct_request.source.cue_sidecar = CueSidecarPolicy::IgnoreCue;
        let direct_metadata =
            crate::convert::processor::dispatch_track_metadata_for_authority_matrix_test(
                &direct_request,
                crate::convert::pipeline::SourceKind::SingleFile,
            )
            .expect("ordinary direct-file metadata must remain readable");
        assert_eq!(
            direct_metadata.extra.get("album").map(String::as_str),
            Some("File Tags Sentinel"),
            "direct conversion metadata must come from the selected file tags",
        );
    ...
    #[test]
    fn directly_queued_audio_without_cue_freezes_ignore_cue_after_admission() {
        let td = tempfile::tempdir().expect("tempdir");
        let audio = td.path().join("ordinary.flac");
        std::fs::write(&audio, b"not-a-real-flac").expect("fixture");
        ...
        assert_eq!(
            decision.cue_sidecar_override,
            Some(CueSidecarPolicy::IgnoreCue),
            "once queue admission proves no viable CUE representation, later source detection must not repeat the same probes",
        );
    }
...
"#;
        ...
        let accepted = coherent_embedded_cuesheet_for_member_audio_with_base_dirs(
