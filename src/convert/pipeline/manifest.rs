...
mod manifest_merge_gap_tests {
    ...
    fn historical_policies_preserve_v1_v2_authority_and_v7_plus_require_v3() {
        ...
        .expect_err("v12 must bind the ordered verification command pipeline");
        assert!(error
            .to_string()
            .contains("missing v1, v2, or v3 executed verification authority"));
        assert!(reference_manifest_with_evidence(
        ...
        .expect_err("v13 must bind the ordered verification command pipeline");
        assert!(error
            .to_string()
            .contains("missing v1, v2, or v3 executed verification authority"));
        assert!(reference_manifest_with_evidence(
        ...
        for (policy, label) in [
            ...
            assert!(
                error
                    .to_string()
                    .contains("missing v1, v2, or v3 executed verification authority"),
                "{label} did not reject missing v3 authority: {error}",
            );
            assert!(reference_manifest_with_evidence(policy, v1, v2, v3).is_ok());
        }
    }
