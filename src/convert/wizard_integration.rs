...
use crate::convert::{
    ConversionManager, ConversionStatus, ConversionOptions,
};
use crate::convert::formats::{
    AudioFormat, QualitySettings, Mp3BitrateMode, AacProfile, WavPackMode,
};
use crate::convert::simple_wizard::{NyquistTransition, DitherType};
...
pub fn extract_wizard_settings(wizard: &tonepoet_wizard::SimpleWizard) -> (AudioFormat, ConversionOptions) {
    ...
    let quality_str = wizard.selected_quality
        .as_ref()
        .map(|s| s.as_str())
        .unwrap_or("Default");
    ...
    log::info!("🎯 Wizard ReplayGain settings: calculate={:?}, mode={:?}",
        wizard.calculate_replaygain, wizard.replaygain_mode);
    ...
    log::info!("🎯 Final calculate_replaygain={} (mode present: {})",
        calculate_replaygain, replaygain_mode.is_some());
    ...
    let options = ConversionOptions {
        ...
        nyquist_transition: if matches!(format,
            AudioFormat::Flac | AudioFormat::Wav | AudioFormat::Aiff |
            AudioFormat::WavPack | AudioFormat::Opus
        ) {
            wizard.nyquist_transition.map(|nt| {
                match nt {
...
pub async fn apply_settings_to_queue(manager: &mut ConversionManager, options: ConversionOptions) {
    ...
    let selected_count = queue.all_items().iter().filter(|item| item.selected).count();

    log::info!("apply_settings_to_queue: has_selection={}, total_items={}, selected_count={}",
               has_selection, total_items, selected_count);
    ...
    for item in queue.all_items_mut() {
        ...
        match item.status {
            ...
            ConversionStatus::Paused => {
                log::info!("Applying settings to item {} (selected={})", item.id, item.selected);
...
pub async fn validate_conversion_ready(manager: &ConversionManager) -> Result<(), String> {
    ...
    if items.is_empty() {
        return Err("No files in conversion queue. Use 'Add Files' or 'Add Folder' first.".to_string());
    }
    
    // Check if we have any items ready to process
    let ready_count = items.iter()
        .filter(|item| matches!(
            item.status, 
    ...
    if ready_count == 0 {
        return Err("No items ready for conversion. All items may be completed or failed.".to_string());
    }
...
pub fn get_conversion_summary(manager: &ConversionManager) -> String {
    ...
    match manager.queue.try_read() {
        Ok(queue) => {
            ...
            if let Some(first_ready) = items.iter()
        ...
        format!(
            ...
            ready, format.name(), processing, completed
        )
    } else {
...
pub fn validate_wizard_selections(wizard: &tonepoet_wizard::SimpleWizard) -> Result<(), String> {
    ...
    match format {
        ...
        tonepoet_wizard::AudioFormat::Wav | tonepoet_wizard::AudioFormat::Aiff => {
            ...
            log::info!("WAV/AIFF format detected - using bit_depth/sample_rate instead of quality string");
        }
        _ => {
...
pub fn preset_to_conversion_options(
    ...
    let quality = match format {
        ...
        AudioFormat::Aac => {
            let profile = settings.aac_profile
                .map(|p| match p {
