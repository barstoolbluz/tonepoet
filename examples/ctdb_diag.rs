...
fn compute_track_crc32(audio: &[i16], is_first: bool, is_last: bool, suffix_skip_i16: usize) -> u32 {
    ...
    let bytes: &[u8] = unsafe {
        std::slice::from_raw_parts(trimmed.as_ptr() as *const u8, trimmed.len() * 2)
    };
...
fn decode_flac_to_i16(path: &PathBuf) -> Vec<i16> {
    ...
    if !output.status.success() {
        panic!("ffmpeg decode failed for {}: {}", path.display(), String::from_utf8_lossy(&output.stderr));
    }
...
fn main() {
    ...
    for path in &paths {
        ...
        println!("  {} -> {} i16 ({} stereo pairs)",
            path.display(), audio.len(), audio.len() / 2);
    ...
    println!("compute_suffix_skip = {} i16 ({} stereo pairs)", suffix_skip, suffix_skip / 2);
    println!("PREFIX_SKIP_I16 = {} ({} stereo pairs)", PREFIX_SKIP_I16, PREFIX_SKIP_I16 / 2);
    println!();
    ...
    for (i, audio) in all_audio.iter().enumerate() {
        ...
        let mark = if format!("{:08x}", crc) == *expected { "✓" } else { "✗" };
        println!("  Track {}: {:08x}  expected {}  {}",
            i + 1, crc, expected, mark);
    }
}
