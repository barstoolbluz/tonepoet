...
const ENTRY_896_SYNDROME_LE_U16: [u16; NPAR] = [
    0x8d1c, 0xf108, 0x4bcc, 0x9ecf, 0xd0a2, 0x8497, 0x77c9, 0x002b,
    0xd5a5, 0xf2b3, 0x5745, 0xe009, 0x469e, 0xf199, 0xe3ad, 0x9f3b,
];

fn decode_flac_to_i16(path: &str) -> Vec<i16> {
    let output = Command::new("ffmpeg")
        .args(["-y", "-hide_banner", "-loglevel", "error", "-i", path,
               "-f", "s16le", "-ar", "44100", "-ac", "2", "-"])
        .output()
        .expect("ffmpeg failed");
    if !output.status.success() {
        panic!("ffmpeg decode failed for {}: {}", path, String::from_utf8_lossy(&output.stderr));
    }
...
fn main() {
    ...
    for t in &tracks { image.extend_from_slice(t); }
    image.extend(std::iter::repeat(0i16).take(STRIDE));
    drop(tracks);
    eprintln!("Disc image: {} i16 ({} stereo pairs)", image.len(), image.len() / 2);

    eprintln!("Computing audio parity matrix (npar={})...", NPAR);
    let codec = tonepoet::ctdb_rs::CtdbCodec::new();
    let parity = tonepoet::ctdb_rs::syndrome::compute_parity_matrix_from_audio(
        codec.galois(), &image, NPAR,
    ).expect("compute parity");

    eprintln!();
    eprintln!("Our audio parity[0]   (16 u16): {}",
        parity[0].iter().take(NPAR).map(|w| format!("{:04x}", w)).collect::<Vec<_>>().join(" "));
    eprintln!("Our audio parity[1]   (16 u16): {}",
        parity[1].iter().take(NPAR).map(|w| format!("{:04x}", w)).collect::<Vec<_>>().join(" "));
    eprintln!("Our audio parity[STRIDE-1] (16 u16): {}",
        parity[STRIDE - 1].iter().take(NPAR).map(|w| format!("{:04x}", w)).collect::<Vec<_>>().join(" "));
    eprintln!();
    eprintln!("896 entry syndrome (target):  {}",
        ENTRY_896_SYNDROME_LE_U16.iter().map(|w| format!("{:04x}", w)).collect::<Vec<_>>().join(" "));
    eprintln!();
    ...
    for &word_offset in &word_offsets {
        ...
        let matches: usize = syn.iter().zip(&ENTRY_896_SYNDROME_LE_U16)
        ...
        if matches == NPAR {
            println!();
            println!("EXACT MATCH at sample offset {:+}:  {}", word_offset / 2,
                syn.iter().map(|w| format!("{:04x}", w)).collect::<Vec<_>>().join(" "));
            return;
        }
    }
    ...
    let entry_syndrome_be: [u16; NPAR] = [
        0x1c8d, 0x08f1, 0xcc4b, 0xcf9e, 0xa2d0, 0x9784, 0xc977, 0x2b00,
        0xa5d5, 0xb3f2, 0x4557, 0x09e0, 0x9e46, 0x99f1, 0xade3, 0x3b9f,
    ];
    eprintln!("896 entry syndrome (BE u16):  {}",
        entry_syndrome_be.iter().map(|w| format!("{:04x}", w)).collect::<Vec<_>>().join(" "));
    for &word_offset in &word_offsets {
        let row_idx = (-word_offset).rem_euclid(STRIDE as i64) as usize;
        let parity_row = &parity[row_idx];
        let gf = codec.galois();
        ...
        let matches: usize = syn.iter().zip(&entry_syndrome_be).filter(|(a, b)| a == b).count();
        if matches >= 4 {
    ...
    for row_idx in 0..STRIDE {
        let parity_row = &parity[row_idx];
        if parity_row.len() < NPAR { continue; }
        let matches: usize = parity_row.iter().take(NPAR)
            .zip(&target).filter(|(a, b)| a == b).count();
        if matches >= NPAR / 2 {
            eprintln!("  parity row {}: {} matches  [{}]",
                row_idx, matches,
                parity_row.iter().take(4).map(|w| format!("{:04x}", w)).collect::<Vec<_>>().join(" "));
        }
    ...
    if let Some((row, m)) = best_row {
        eprintln!("Best parity-row match: row {} with {} of {} symbols", row, m, NPAR);
    } else {
        eprintln!("No partial parity-row matches (best <{} symbols).", NPAR / 2);
    }
    ...
    for col in 0..NPAR {
        let column_slice: Vec<u16> = parity.iter().take(NPAR)
            .map(|row| row[col]).collect();
        let matches: usize = column_slice.iter().zip(&target).filter(|(a, b)| a == b).count();
        if matches >= NPAR / 2 {
        ...
        match bm {
            Some((_sigma, errors_found)) => {
                if errors_found <= max_correctable {
                    eprintln!("  word_offset {:+} (row {}): BM found {} errors (≤ {}) ← CORRECTABLE",
