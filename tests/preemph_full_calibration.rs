...
async fn full_calibration() {
    ...
    if !pe_llrs.is_empty() {
        ...
        println!(
            "  PE files ({} scored, {} errors):",
            pe_llrs.len(),
            pe_errors
        );
        println!(
            "    LLR:   mean={:+.2}, min={:+.2}, max={:+.2}",
            mean, min, max
        );
    ...
    for (album, pe_llrs) in &pe_by_album {
        if let Some(non_pe_llrs) = non_pe_by_album.get(album) {
            ...
            println!(
                "  {:45} PE={:+7.2} ({:2}t)  non-PE={:+7.2} ({:2}t)  Δ={:+.2}",
                album,
                pe_mean,
                pe_llrs.len(),
                non_pe_mean,
