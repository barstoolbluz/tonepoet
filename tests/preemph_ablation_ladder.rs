...
fn compute_track(path: &PathBuf, corpus_model: &corpus::CorpusModel) -> Option<TrackData> {
...
fn train_logreg(samples: &[(&[f64], bool)], n_features: usize, lambda: f64) -> (Vec<f64>, f64) {
    ...
    let std_samples: Vec<(Vec<f64>, f64)> = samples
        .iter()
        .map(|(x, l)| {
            let z: Vec<f64> = (0..n_features)
                .map(|i| (x[i] - means[i]) / stds[i])
                .collect();
            (z, if *l { 1.0 } else { 0.0 })
        })
        .collect();
...
fn evaluate_album_model(
    ...
    for tracks in pe_albums.values() {
        for t in tracks {
            track_samples.push((track_feature_fn(t), true));
        }
    }
    for tracks in np_albums.values() {
        for t in tracks {
            track_samples.push((track_feature_fn(t), false));
        }
    }

    let refs: Vec<(&[f64], bool)> = track_samples
        .iter()
    ...
    for (_, tracks) in pe_albums {
        let scores: Vec<f64> = tracks
            .iter()
            .map(|t| score_logreg(&track_feature_fn(t), &weights, bias))
            .collect();
        if scores.len() >= 3 {
    ...
    let recall = if pe_total > 0 {
        tp as f64 / pe_total as f64
    } else {
        0.0
    };
    let fpr = if neg_total > 0 {
