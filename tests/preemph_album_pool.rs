...
fn score_track_full(
    ...
    let shape =
        models::TrackShapeFeatures::new(&multi_alpha, model_scores.pe_correlation, deemph_delta);
...
async fn ab_ablation() {
    ...
    for (album, tracks) in &pe_albums {
        let summaries: Vec<scoring::TrackSummary> =
            tracks.iter().map(|t| t.summary.clone()).collect();
    ...
    println!(
        "  Training track shape classifier on {} tracks...",
        track_samples.len()
    );
    let track_clf = match scoring::TrackPeClassifier::train(&track_samples, 0.1) {
        Ok(c) => c,
        Err(e) => {
            println!("  Track classifier failed: {}", e);
            return;
        }
    };
    ...
    let mut b_album_samples: Vec<([f64; scoring::NUM_POOLED_ALBUM_FEATURES], bool, String)> =
        Vec::new();
    ...
    match (&a_result, &b_result) {
        (Ok((a_clf, a_acc, a_fpr, a_prec)), Ok((b_clf, b_acc, b_fpr, b_prec))) => {
            println!(
                "  Pipeline A (raw features):   accuracy={:.1}% FPR={:.1}% precision={:.1}%",
                a_acc * 100.0,
                a_fpr * 100.0,
                a_prec * 100.0
            );
            println!(
                "  Pipeline B (shape → pooled):  accuracy={:.1}% FPR={:.1}% precision={:.1}%",
                b_acc * 100.0,
                b_fpr * 100.0,
                b_prec * 100.0
            );
            ...
            for (album, tracks) in &pe_albums {
                ...
                let a_det = scoring::extract_album_features(&summaries)
                    .map(|f| a_clf.predict(&f))
                    .unwrap_or(false);
                let track_scores: Vec<f64> =
                    tracks.iter().map(|t| track_clf.score(&t.shape)).collect();
                let b_det = scoring::extract_pooled_album_features(&track_scores)
                    .map(|f| b_clf.predict(&f))
                    .unwrap_or(false);
            ...
            println!(
                ...
                a_pe_det,
                pe_total,
                a_pe_det as f64 / pe_total as f64 * 100.0,
                a_np_fp,
                np_total,
                a_np_fp as f64 / np_total as f64 * 100.0
            );
            println!(
                "  B: PE {}/{} ({:.0}%), FP {}/{} ({:.1}%)",
                b_pe_det,
                pe_total,
                b_pe_det as f64 / pe_total as f64 * 100.0,
                b_np_fp,
                np_total,
                b_np_fp as f64 / np_total as f64 * 100.0
            );
    ...
    for (pe_album, pe_tracks) in &pe_albums {
        ...
        for (np_album, np_tracks) in &np_albums {
            if album_key(np_album) == pe_key {
                let pe_summaries: Vec<scoring::TrackSummary> =
                    pe_tracks.iter().map(|t| t.summary.clone()).collect();
                let np_summaries: Vec<scoring::TrackSummary> =
                    np_tracks.iter().map(|t| t.summary.clone()).collect();
                let (pe_conf, _) = scoring::album_pool_soft(&pe_summaries);
                let (np_conf, _) = scoring::album_pool_soft(&np_summaries);
                println!(
                    "  {:45} PE={:?}  nonPE={:?}",
                    &pe_key[..pe_key.len().min(45)],
                    pe_conf,
                    np_conf
                );
            }
