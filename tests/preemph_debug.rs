...
async fn debug_spectral_shapes() {
    use tonepoet::tui::preemphasis::{corpus, frame_select, iir, stft};
    ...
    let pe_curve: Vec<f64> = centers
        .iter()
        .map(|&f| iir::theoretical_gain_db(f))
        .collect();

    println!(
        "\n{:>8} {:>8} {:>8} {:>8} {:>8} {:>8}",
        "freq", "corpus", "PE_file", "nonPE", "PE-corp", "PE_theo"
    );
    println!("{}", "-".repeat(56));
    for k in 0..stft::NUM_BANDS {
        let diff_pe = pe_median[k] - corpus_model.mean[k];
        println!(
            "{:8.0} {:8.2} {:8.2} {:8.2} {:+8.2} {:8.2}",
            centers[k], corpus_model.mean[k], pe_median[k], non_pe_median[k], diff_pe, pe_curve[k]
        );
    }
    ...
    let diff: Vec<f64> = (0..stft::NUM_BANDS)
        .map(|k| pe_median[k] - corpus_model.mean[k])
        .collect();
