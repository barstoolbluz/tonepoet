...
async fn debug_fold_assignment() {
    ...
    let dummy_features = |alpha: f64, rng: &mut dyn FnMut() -> f64| scoring::TrackFeatures {
        features: [alpha + rng() * 0.2 - 0.1, rng() * 0.3, rng() * 0.5 - 0.25, 3.5 + rng(), 0.05 + rng() * 0.1],
        alpha,
    ...
    for (_, label, group) in &samples {
        ...
        if *label { entry.0 += 1; } else { entry.1 += 1; }
    }

    println!("\n=== {} total samples, {} groups ===", samples.len(), groups.len());
    ...
    println!("\nPE groups: {}, non-PE groups: {}", pe_groups, non_pe_groups);

    // Check eligibility.
    let eligible = samples.iter().filter(|(f, _, _)| {
        ...
        let unstable = !f.alpha_stability_missing
            && (f.features[4] > f.alpha.abs() * 3.0);
    ...
    // Manually test fold assignment for 3-fold.
    {
        ...
        let mut pe_g: Vec<(&str, usize)> = by_group.iter()
            .filter(|(_, (pe, _))| *pe > 0)
            .map(|(g, (pe, _))| (g.as_str(), *pe))
            .collect();
        let mut np_g: Vec<(&str, usize)> = by_group.iter()
            .filter(|(_, (_, np))| *np > 0)
        ...
        match scoring::grouped_cv_train_with_calibration_report(&samples, k, 0.01) {
            Ok((classifier, report)) => {
                println!("\n{}-fold CV: OK — accuracy={:.1}%, FPR={:.1}%, threshold={:.4}",
                    k, report.metrics.track_accuracy * 100.0,
