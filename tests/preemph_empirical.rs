...
fn score_file(path: &PathBuf) -> Result<(f64, f64, f64, f64, usize), String> {
    ...
    if info.sample_rate > 48000 { return Err("hi-res".into()); }
    ...
    if selected.frames.is_empty() { return Err("no frames".into()); }
    ...
    Ok((scores.z_score, scores.alpha, scores.pe_correlation, deemph_delta, selected.frames.len()))
}
...
async fn empirical_template_test() {
    ...
    if !pe_dir.is_dir() || !deemph_dir.is_dir() || !non_pe_dir.is_dir() {
        eprintln!("SKIP: directories not found"); return;
    }
    ...
    if corpus::load_corpus().is_err() {
        ...
        corpus::train_corpus_from_dir(&non_pe_dir).await.expect("corpus training failed");
    }
    let corpus_model = corpus::load_corpus().unwrap();
    println!("  Corpus: {} tracks, {} frames", corpus_model.n_tracks, corpus_model.n_frames);
    ...
    let emp_usable: Vec<f64> = s_emp.iter().zip(mask.iter())
        .filter(|(_, &m)| m).map(|(&v, _)| v).collect();
    let theory_usable: Vec<f64> = s_theory.iter().zip(mask.iter())
        .filter(|(_, &m)| m).map(|(&v, _)| v).collect();
    ...
    for k in 0..NUM_BANDS {
        if !mask[k] { continue; }
        println!("  {:8.0} {:10.3} {:10.3}", centers[k], s_emp[k], s_theory[k]);
    }
    ...
    for (label, files) in [
        ("PE", &pe_sample), ("Deemphasized", &de_sample), ("Non-PE", &np_sample)
    ] {
        let mut zs = Vec::new();
        ...
        for path in files.iter() {
            match tokio::task::spawn_blocking({
                ...
                Ok((z, a, r, d, _)) => { zs.push(z); alphas.push(a); corrs.push(r); deltas.push(d); }
                Err(_) => {}
            }
        }
        ...
        println!("  {:15} n={:3}  z={:+6.2} [{:+.2},{:+.2}]  α={:+.3}  r={:+.3}  Δd={:+.3}",
