...
fn diagnose_track(
    ...
    let per_frame_alphas = compute_per_frame_alphas(
        &selected.frames, &stft_result, corpus_model, &mask, &pe_template,
    );
    ...
    let all_frame_alphas = compute_per_frame_alphas(
        &all_frame_indices, &stft_result, corpus_model, &mask, &pe_template,
    );
...
fn compute_per_frame_alphas(
    ...
    for &idx in frame_indices {
        ...
        let residual: Vec<f64> = data.iter().enumerate()
            .map(|(i, &y)| y - slope * i as f64 - intercept).collect();
...
async fn error_analysis() {
    ...
    if !pe_dir.is_dir() { eprintln!("SKIP"); return; }
    ...
    for (album, tracks) in &albums {
        ...
        let median_alpha = if alphas.len() % 2 == 1 { alphas[alphas.len() / 2] }
            else { (alphas[alphas.len() / 2 - 1] + alphas[alphas.len() / 2]) / 2.0 };
    ...
    println!("\n=== {} detected, {} missed PE albums ===", detected.len(), missed.len());

    // Compare distributions.
    println!("\n=== DETECTED PE ALBUMS ===");
    println!("{:55} {:>5} {:>7} {:>7} {:>7} {:>7} {:>7} {:>7}",
    ...
    for (album, tracks) in &missed {
        ...
        let median_all = if all_alphas.len() % 2 == 1 { all_alphas[all_alphas.len() / 2] }
            else { (all_alphas[all_alphas.len() / 2 - 1] + all_alphas[all_alphas.len() / 2]) / 2.0 };
        let frac_pos_all = tracks.iter().filter(|t| t.alpha_all_frames > 0.0).count() as f64
            / tracks.len() as f64;
        let would_detect = median_all > 0.0 && frac_pos_all >= 0.60 && tracks.len() >= 3;
        if would_detect {
            recovered += 1;
            let short = &album[..album.len().min(55)];
            println!("  RECOVERED: {:55} median_all={:+.3} frac+={:.0}%",
                short, median_all, frac_pos_all * 100.0);
        }
    }
    println!("  {} of {} missed albums recovered with all-frame alpha", recovered, missed.len());
    ...
    for (album, tracks) in &missed {
        ...
        let median_tq = if tq_alphas.len() % 2 == 1 { tq_alphas[tq_alphas.len() / 2] }
            else { (tq_alphas[tq_alphas.len() / 2 - 1] + tq_alphas[tq_alphas.len() / 2]) / 2.0 };
        let frac_pos_tq = tracks.iter().filter(|t| t.alpha_p75 > 0.0).count() as f64
            / tracks.len() as f64;
        let would_detect = median_tq > 0.0 && frac_pos_tq >= 0.60 && tracks.len() >= 3;
        if would_detect {
            recovered_tq += 1;
            let short = &album[..album.len().min(55)];
            println!("  RECOVERED: {:55} median_p75={:+.3} frac+={:.0}%",
                short, median_tq, frac_pos_tq * 100.0);
        }
    }
    println!("  {} of {} missed albums recovered with p75 alpha", recovered_tq, missed.len());
}
...
fn print_album_summary(album: &str, tracks: &[TrackDiag]) {
    ...
    let mut alphas: Vec<f64> = tracks.iter().map(|t| t.alpha).collect();
    alphas.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    let median_alpha = if n % 2 == 1 { alphas[n / 2] } else { (alphas[n / 2 - 1] + alphas[n / 2]) / 2.0 };
    ...
    println!("{:55} {:>5} {:>+7.3} {:>+7.3} {:>+7.3} {:>+7.3} {:>6.0}% {:>+7.3}",
        short, n, median_alpha, mean_p75, mean_top, mean_all, frac_pos * 100.0, mean_pe_corr);
}
...
fn print_aggregate(label: &str, albums: &[(String, Vec<TrackDiag>)]) {
    ...
    println!("  {:12}: {} albums, {} tracks, mean_frames={:.0}", label, albums.len(), n, mean_frames);
    println!("    alpha: quiet_med={:+.3}, p75={:+.3}, top_q={:+.3}, all_frames={:+.3}, frac+={:.0}%",
        mean_alpha, mean_p75, mean_top, mean_all, frac_pos * 100.0);
