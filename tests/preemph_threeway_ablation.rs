...
use tonepoet::tui::preemphasis::{corpus, frame_select, models, stft};
...
fn compute_multi_summary(
    ...
    let quiet_alphas = compute_per_frame_alphas(
        &selected.frames,
        &stft_result,
        corpus_model,
        &mask,
    ...
    let all_alphas = compute_per_frame_alphas(
        &all_indices,
        &stft_result,
        corpus_model,
        &mask,
...
fn album_aggregate(tracks: &[TrackMultiSummary]) -> Option<AlbumStats> {
    ...
    Some(AlbumStats {
        ...
        frac_tracks_quiet_p75_positive: tracks.iter().filter(|t| t.quiet_p75_alpha > 0.0).count()
            as f64
            / n as f64,
...
async fn threeway_summary_ablation() {
    ...
    for (label, dir, albums) in [
        ...
        if !dir.is_dir() {
            println!("  Skipping {} (dir not found)", label);
            continue;
        }
    ...
    let pe_agg: Vec<AlbumStats> = pe_albums
        .values()
        .filter_map(|t| album_aggregate(t))
        .collect();
    let de_agg: Vec<AlbumStats> = de_albums
        .values()
        .filter_map(|t| album_aggregate(t))
        .collect();
    let np_agg: Vec<AlbumStats> = np_albums
        .values()
        .filter_map(|t| album_aggregate(t))
        .collect();
    ...
    for (stat_name, extractor) in [
        ...
        ("frac_p75_positive", |a: &AlbumStats| {
            a.frac_tracks_quiet_p75_positive
        }),
        ...
        for (group_name, agg) in [("PE", &pe_agg), ("Deemph", &de_agg), ("Non-PE", &np_agg)] {
            ...
            let p75 = s[3 * s.len() / 4];
            println!(
                "{:22} {:>8} {:>+10.3} {:>+10.3} {:>+10.3} {:>+10.3} {:>+10.3} {:>+10.3}",
    ...
    println!(
        "{:22} {:>10} {:>10} {:>10} {:>10}",
        "Statistic", "PE_median", "NP_median", "Gap", "Overlap?"
    );
    println!("{}", "-".repeat(66));
    ...
    for (stat_name, extractor) in [
        ...
        let mut pe_vals: Vec<f64> = pe_agg
            .iter()
            .map(|a| extractor(a))
            .filter(|v| v.is_finite())
            .collect();
        let mut np_vals: Vec<f64> = np_agg
            .iter()
            .map(|a| extractor(a))
            .filter(|v| v.is_finite())
            .collect();
        ...
        if pe_vals.is_empty() || np_vals.is_empty() {
            continue;
        }
    ...
    for (stat_name, med_ext, frac_ext) in [
        ...
        println!(
            "  {}: PE={}/{} ({:.0}%), Non-PE FP={}/{} ({:.1}%), Deemph={}/{} ({:.0}%)",
            stat_name,
            pe_det,
            pe_agg.len(),
            pe_det as f64 / pe_agg.len().max(1) as f64 * 100.0,
            np_fp,
            np_agg.len(),
            np_fp as f64 / np_agg.len().max(1) as f64 * 100.0,
            de_det,
            de_agg.len(),
            de_det as f64 / de_agg.len().max(1) as f64 * 100.0
        );
    }
}
