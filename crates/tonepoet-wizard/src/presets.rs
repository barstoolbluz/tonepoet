...
use crate::types::{AudioFormat, DitherType, NyquistTransition, OpusContentType, AacProfile, ReplayGainMode};
...
impl PresetManager {
    ...
    fn get_presets_directory() -> io::Result<PathBuf> {
        ...
        if let Ok(xdg_config) = std::env::var("XDG_CONFIG_HOME") {
            ...
            Ok(PathBuf::from(home).join(".config").join("tonepoet").join("presets"))
        } else {
    ...
    pub fn load_preset(&self, name: &str) -> io::Result<ConversionPreset> {
        ...
        let preset: ConversionPreset = toml::from_str(&toml_str)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e))?;
