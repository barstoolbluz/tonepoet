...
pub enum PopupType {
    PresetName,
    #[allow(dead_code)]
    TextInput { field: EditingField },
    OverwriteConfirm { preset_name: String },
    PresetList { presets: Vec<String>, selected_index: usize },
    FileBrowser(Box<FileBrowser>),
    NewFolder { parent_path: PathBuf },
}
...
impl Default for SimpleWizard {
    fn default() -> Self {
        Self {
            ...
            copy_files_extensions: "txt, cue, log, nfo, pdf, png, jpg, jpeg, gif, tif, tiff, bmp, webp".to_string(),
...
impl SimpleWizard {
    ...
    pub fn get_aac_bitrates(&self) -> Vec<&'static str> {
        match self.aac_profile {
            Some(AacProfile::LcAac) => vec!["320 kbps", "256 kbps", "192 kbps", "160 kbps", "128 kbps", "96 kbps", "64 kbps"],
            Some(AacProfile::HeAac) => vec!["128 kbps", "96 kbps", "64 kbps", "48 kbps", "32 kbps"],
            Some(AacProfile::HeAacV2) => vec!["96 kbps", "64 kbps", "48 kbps", "32 kbps", "24 kbps"],
            None => vec!["320 kbps", "256 kbps", "192 kbps", "160 kbps", "128 kbps", "96 kbps", "64 kbps"], // Default to LC-AAC
        }
    }
        ...
        if self.current_step < 4 {
            ...
            if self.current_step == 1 && self.selected_quality.is_none() {
                let default_quality = match self.selected_format {
                    ...
                    Some(AudioFormat::Wav) | Some(AudioFormat::Aiff) | Some(AudioFormat::WavPack) => None, // Lossless formats don't need quality
                    None => None,
                };
    ...
    pub fn show_destination_browser(&mut self) {
        ...
        let start_path = match &self.destination_mode {
            ...
            DestinationMode::AskEveryTime => std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from(".")),
        };
...
impl FileBrowser {
    ...
    pub fn refresh_entries(&mut self) {
        ...
        if let Some(parent) = self.current_path.parent() {
            ...
            let parent_path = parent.canonicalize().unwrap_or_else(|_| parent.to_path_buf());
