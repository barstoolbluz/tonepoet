...
fn main() {
    ...
    println!("  Input: resample_quality = {:?}", test_item.options.resample_quality);
    println!("  Output: resample_quality = {:?}", mapped_settings.resample_quality);
    ...
    println!("  Input: compression_mode = {:?}", MainWavPackMode::VeryHigh);
    println!("  Output: compression_level = {:?}", wavpack_mapped.compression_level);
    ...
    match builder.build(Path::new("input.wav"), Path::new("output.aiff"), &mapped_settings) {
        Ok(command) => {
