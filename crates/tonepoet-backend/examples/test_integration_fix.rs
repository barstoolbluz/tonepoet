...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  resample_quality: {:?} ← CRITICAL: This should be Some(0), not None!", settings.resample_quality);
    println!();
    ...
    match pipeline_builder.build_pipeline(Path::new("input.flac"), Path::new("output.aiff"), &settings) {
        Ok(pipeline) => {
