...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    if !availability.missing_critical_tools.is_empty() {
        println!(
            ...
            availability.missing_critical_tools
        );
    }
    ...
    println!(
        "  Test item: {} -> {:?} (compression: 8, resample: HQ)",
    ...
    if !progress_updates.is_empty() {
        ...
        let all_in_range = progress_updates
            .iter()
            .all(|update| update.progress >= 40.0 && update.progress <= 90.0);
