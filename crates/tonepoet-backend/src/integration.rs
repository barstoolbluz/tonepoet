...
pub fn map_conversion_item_to_settings(item: &ConversionItem) -> ConversionSettings {
    ...
    let (bit_depth, compression_level, mp3_bitrate, mp3_quality, mp3_mode, aac_profile) =
        match &item.options.quality {
            ...
            MainQualitySettings::Wav {
                bit_depth,
                sample_rate: _,
            } => {
                let mapped_bit_depth = if *bit_depth == 0 {
                    None
                } else {
            ...
            MainQualitySettings::Mp3 {
                bitrate_mode,
                quality: _,
            } => {
                let (bitrate, mp3_quality, mode) = match bitrate_mode {
                    MainMp3BitrateMode::Cbr { bitrate } => {
                        (Some(*bitrate), None, Some(Mp3Mode::Cbr))
                    }
                    MainMp3BitrateMode::Vbr { quality } => {
                        (None, Some(*quality), Some(Mp3Mode::Vbr))
                    }
                    MainMp3BitrateMode::Abr { bitrate } => {
                        (Some(*bitrate), None, Some(Mp3Mode::Abr))
                    }
                };
            ...
            MainQualitySettings::Opus {
                bitrate,
                complexity: _,
            } => (None, None, Some(*bitrate), None, None, None),
            MainQualitySettings::WavPack {
                compression_mode,
                hybrid_mode: _,
                correction_file: _,
            } => {
            ...
            MainQualitySettings::Alac => (None, None, None, None, None, None),
        };
    ...
    log::debug!(
        "🎯 Bit depth resolution: quality={:?}, target={:?}, final={:?}",
        bit_depth_from_quality,
        item.options.target_bit_depth,
        bit_depth
    );
    ...
    ConversionSettings {
        ...
        // Processing options
        dither_type: item.options.dither_type.map(|dt| match dt {
        ...
        nyquist_transition: item.options.nyquist_transition.map(|nt| match nt {
...
pub enum MainQualitySettings {
    ...
    WavPack {
        compression_mode: MainWavPackMode,
        hybrid_mode: bool,
        correction_file: bool,
    },
    Mp3 {
        bitrate_mode: MainMp3BitrateMode,
        quality: u8,
    },
    Aac {
        bitrate: u32,
        profile: MainAacProfile,
    },
    Opus {
        bitrate: u32,
        complexity: u8,
...
pub fn calculate_phase_progress(stage_progress: f32, target_phase: ConversionPhase) -> (f32, f32) {
