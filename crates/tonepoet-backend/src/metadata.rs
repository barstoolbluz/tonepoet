...
impl FlacMetadataExtractor {
    ...
    pub fn extract(&self, flac_file: &Path) -> Result<FlacMetadata> {
        ...
        let output = Command::new("metaflac")
            ...
            .output()
            .map_err(|e| {
                ConversionError::BackendUnavailable(format!("Failed to run metaflac: {}", e))
            })?;
        ...
        log::info!(
            "Extracted {} metadata fields from {}",
...
impl FlacMetadataApplier {
    ...
    pub fn apply(&self, metadata: &FlacMetadata, flac_file: &Path) -> Result<()> {
        ...
        let mut child = cmd
            ...
            .spawn()
            .map_err(|e| {
                ConversionError::BackendUnavailable(format!("Failed to spawn metaflac: {}", e))
            })?;
        ...
        if let Some(stdin) = child.stdin.take() {
            use std::io::Write;
            let mut stdin = stdin;
            stdin.write_all(tags_text.as_bytes()).map_err(|e| {
                ConversionError::InvalidSettings(format!("Failed to write tags to metaflac: {}", e))
            })?;
        }

        let output = child.wait_with_output().map_err(|e| {
            ConversionError::BackendUnavailable(format!("Failed to wait for metaflac: {}", e))
        })?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(ConversionError::InvalidSettings(format!(
                "metaflac tag application failed: {}",
                stderr
            )));
        }

        log::info!(
            "Applied {} metadata fields to {}",
            metadata.total_field_count(),
            flac_file.display()
        );
    ...
    pub fn verify(&self, metadata: &FlacMetadata, flac_file: &Path) -> Result<bool> {
        ...
        let original_comment = metadata
            .comment
            .as_ref()
            .or_else(|| metadata.custom_fields.get("DESCRIPTION"))
            .or_else(|| metadata.custom_fields.get("description"));

        let reextracted_comment = reextracted
            .comment
            .as_ref()
        ...
        if !success {
            log::warn!(
                "FLAC metadata verification failed for {}: {}/{} checks passed ({:.1}%)",
                flac_file.display(),
                matches,
                total_checks,
                success_rate * 100.0
            ...
            log::info!(
                "FLAC metadata verification passed: {}/{} checks passed ({:.1}%)",
                matches,
                total_checks,
                success_rate * 100.0
            );
...
impl MetadataPreservingPipeline {
    ...
    pub fn execute_with_metadata_preservation(
        ...
        let metadata = if input.extension().map_or(false, |ext| ext == "flac") {
            ...
            log::info!(
                "Extracting metadata from WavPack input: {}",
                input.display()
            );
            ...
            log::info!(
                "Extracting metadata from AAC/M4A input: {}",
                input.display()
            );
        ...
        if let Some(ref metadata) = metadata {
            ...
            let json_content = serde_json::to_string_pretty(metadata).map_err(|e| {
                ConversionError::InvalidSettings(format!("Failed to serialize metadata: {}", e))
            })?;

            fs::write(&metadata_file, json_content).map_err(|e| {
                ConversionError::InvalidSettings(format!("Failed to write metadata file: {}", e))
            })?;

            self.metadata_file = Some(metadata_file);
            log::info!(
                "Saved metadata to {}",
                self.metadata_file.as_ref().unwrap().display()
            );
        }
        ...
        for (i, command) in pipeline.commands.iter().enumerate() {
            ...
            let output = command.execute().map_err(|e| {
                ConversionError::InvalidSettings(format!("Pipeline stage {} failed: {}", i + 1, e))
            })?;
        ...
        if let Some(metadata) = metadata {
            if output.extension().map_or(false, |ext| ext == "flac") {
                ...
                log::info!(
                    "Reapplying metadata to WavPack output: {}",
                    output.display()
                );
                ...
                if !self.wv_applier.verify(&metadata, output)? {
                    log::error!(
                        "WavPack metadata verification failed for {}",
                        output.display()
                    );
                    return Err(ConversionError::InvalidSettings(
                ...
                log::info!(
                    "Reapplying metadata to AAC/M4A output: {}",
                    output.display()
                );
...
mod tests {
    ...
    fn test_metadata_round_trip() {
        ...
        assert_eq!(
            empty_metadata.total_field_count(),
            0,
            "Tags not fully removed"
        );

        // Reapply metadata
        applier
            .apply(&metadata, &temp_flac)
            .expect("Failed to apply metadata");

        // Verify restoration
        assert!(
            applier
                .verify(&metadata, &temp_flac)
                .expect("Verification failed"),
            "Metadata round-trip failed"
        );

        let restored_metadata = extractor
            .extract(&temp_flac)
            .expect("Failed to extract restored metadata");
        assert_eq!(
            restored_metadata.total_field_count(),
            original_count,
            "Field count mismatch after round-trip"
        );

        println!(
            "Round-trip test passed: {} fields preserved",
            original_count
        );
    }
}
...
impl WavPackMetadataExtractor {
    ...
    pub fn extract(&self, wv_file: &Path) -> Result<FlacMetadata> {
        ...
        let output = Command::new("ffprobe")
            ...
            .output()
            .map_err(|e| {
                ConversionError::BackendUnavailable(format!("Failed to run ffprobe: {}", e))
            })?;
        ...
        let ffprobe_data: FFprobeOutput = serde_json::from_str(&json_text).map_err(|e| {
            ConversionError::InvalidSettings(format!("Failed to parse ffprobe JSON: {}", e))
        })?;
        ...
        log::info!(
            "Extracted {} WavPack metadata fields from {}",
            metadata.total_field_count(),
            wv_file.display()
        );
...
impl WavPackMetadataApplier {
    ...
    pub fn apply(&self, metadata: &FlacMetadata, wv_file: &Path) -> Result<()> {
        ...
        let temp_file = wv_file.with_file_name(format!(
            "{}_temp.wv",
            wv_file.file_stem().unwrap().to_string_lossy()
        ));
        ...
        let output = Command::new("ffmpeg").args(&args).output().map_err(|e| {
            ConversionError::BackendUnavailable(format!(
                "Failed to run ffmpeg for WavPack metadata: {}",
                e
            ))
        })?;
        ...
        if !output.status.success() {
            ...
            return Err(ConversionError::InvalidSettings(format!(
                "FFmpeg WavPack metadata application failed: {}",
                stderr
            )));
        }

        // Replace original with tagged version
        fs::rename(&temp_file, wv_file).map_err(|e| {
            ConversionError::InvalidSettings(format!(
                "Failed to replace WavPack file with tagged version: {}",
                e
            ))
        })?;

        log::info!(
            "Applied {} metadata fields to WavPack file {}",
            metadata.total_field_count(),
            wv_file.display()
        );
    ...
    pub fn verify(&self, metadata: &FlacMetadata, wv_file: &Path) -> Result<bool> {
        ...
        if !success {
            log::warn!(
                "WavPack metadata verification failed for {}: {}/{} fields matched ({:.1}%)",
                wv_file.display(),
                matches,
                total_checks,
                success_rate * 100.0
            ...
            log::info!(
                "WavPack metadata verification passed: {}/{} fields matched ({:.1}%)",
                matches,
                total_checks,
                success_rate * 100.0
            );
...
impl OpusMetadataExtractor {
    ...
    pub fn extract(&self, opus_file: &Path) -> Result<FlacMetadata> {
        ...
        let output = Command::new(&opustags_path)
            ...
            .output()
            .map_err(|e| {
                ConversionError::BackendUnavailable(format!("Failed to run opustags: {}", e))
            })?;
        ...
        log::info!(
            "Extracted {} Opus metadata fields from {}",
            metadata.total_field_count(),
            opus_file.display()
        );
...
impl OpusMetadataApplier {
    ...
    pub fn apply(&self, metadata: &FlacMetadata, opus_file: &Path) -> Result<()> {
        ...
        let mut child = cmd
            ...
            .spawn()
            .map_err(|e| {
                ConversionError::BackendUnavailable(format!("Failed to spawn opustags: {}", e))
            })?;
        ...
        if let Some(stdin) = child.stdin.take() {
            ...
            let mut stdin = stdin;
            stdin.write_all(tags_text.as_bytes()).map_err(|e| {
                ConversionError::InvalidSettings(format!("Failed to write tags to opustags: {}", e))
            })?;
        }

        let output = child.wait_with_output().map_err(|e| {
            ConversionError::BackendUnavailable(format!("Failed to wait for opustags: {}", e))
        })?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(ConversionError::InvalidSettings(format!(
                "opustags tag application failed: {}",
                stderr
            )));
        }

        log::info!(
            "Applied {} metadata fields to Opus file {}",
            metadata.total_field_count(),
            opus_file.display()
        );
    ...
    pub fn verify(&self, metadata: &FlacMetadata, opus_file: &Path) -> Result<bool> {
        ...
        if !success {
            log::warn!(
                "Opus metadata verification failed for {}: {}/{} fields matched ({:.1}%)",
                opus_file.display(),
                matches,
                total_checks,
                success_rate * 100.0
            ...
            log::info!(
                "Opus metadata verification passed: {}/{} fields matched ({:.1}%)",
                matches,
                total_checks,
                success_rate * 100.0
            );
...
impl AacMetadataExtractor {
    ...
    pub fn extract(&self, m4a_file: &Path) -> Result<FlacMetadata> {
        ...
        let output = Command::new(&ap_path)
            ...
            .output()
            .map_err(|e| {
                ConversionError::InvalidSettings(format!("Failed to run AtomicParsley: {}", e))
            })?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(ConversionError::InvalidSettings(format!(
                "AtomicParsley extraction failed: {}",
                stderr
            )));
        }
                            ...
                            if let Some(value_start) = line.find("contains: ") {
                                ...
                                match field_name {
                                    "replaygain_track_gain" | "REPLAYGAIN_TRACK_GAIN" => {
                                        metadata
                                            .custom_fields
                                            .insert("REPLAYGAIN_TRACK_GAIN".to_string(), value);
                                    }
                                    "replaygain_track_peak" | "REPLAYGAIN_TRACK_PEAK" => {
                                        metadata
                                            .custom_fields
                                            .insert("REPLAYGAIN_TRACK_PEAK".to_string(), value);
                                    }
                                    "replaygain_album_gain" | "REPLAYGAIN_ALBUM_GAIN" => {
                                        metadata
                                            .custom_fields
                                            .insert("REPLAYGAIN_ALBUM_GAIN".to_string(), value);
                                    }
                                    "replaygain_album_peak" | "REPLAYGAIN_ALBUM_PEAK" => {
                                        metadata
                                            .custom_fields
                                            .insert("REPLAYGAIN_ALBUM_PEAK".to_string(), value);
                                    }
                                    _ => {
        ...
        metadata.original_field_count = field_count;
        log::info!(
            "Extracted {} metadata fields from AAC/M4A file",
            field_count
        );
