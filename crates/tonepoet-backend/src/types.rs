...
impl ConversionCommand {
                ...
                None => {
                    ...
                    if start_time.elapsed() > timeout_duration {
                        ...
                        return Err(std::io::Error::new(
                            ...
                            format!(
                                "Command '{}' timed out after {} seconds. Args: {:?}",
    ...
    pub fn estimate_duration(
        &mut self,
        input_path: &Path,
    ) -> Result<DurationEstimate, crate::ConversionError> {
        ...
        if let Ok(metadata) = Self::get_audio_metadata(input_path) {
            if let Some(duration) = metadata.duration {
                ...
                let estimated_duration =
                    Duration::from_secs_f64(duration * complexity_factor as f64);
        ...
        if let Ok(file_metadata) = std::fs::metadata(input_path) {
            ...
            let estimate = DurationEstimate {
                ...
                method: EstimationMethod::FileSize {
                    file_size,
                    processing_speed,
                },
            };
    ...
    pub fn calculate_complexity_factor(&self) -> f32 {
        ...
        if self
            .arguments
            .iter()
            .any(|arg| arg.contains("aresample") || arg.contains("rate"))
        {
            factor *= 1.5;
        }
        ...
        if self
            .arguments
            .iter()
            .any(|arg| arg.contains("precision=32") || arg.contains("-v"))
        {
            factor *= 2.0;
        }
        ...
        if self
            .arguments
            .iter()
            .any(|arg| arg.contains("libmp3lame") || arg.contains("libopus"))
        {
            factor *= 1.3;
        }
