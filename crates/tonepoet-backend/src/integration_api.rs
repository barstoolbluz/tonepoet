...
impl ConversionBackend {
    ...
    pub async fn convert_item(
        ...
        self.convert_item_with_settings(item, input_path, output_path, progress_tx, settings)
            .await
    }
    ...
    async fn convert_item_with_settings(
        ...
        let pipeline = self
            .pipeline_builder
            .build_pipeline(input_path, output_path, &settings)?;
        ...
        let _outputs = pipeline
            .execute_with_phase_progress(progress_tx, &item.id, ConversionPhase::Converting)
            .await?;
...
impl ToolAvailability {
    ...
    pub fn supports_format_specific_encoding(&self, format: AudioFormat) -> bool {
        match format {
            AudioFormat::Flac => self.available_tools.get("flac").copied().unwrap_or(false),
            AudioFormat::Opus => self
                .available_tools
                .get("opusenc")
                .copied()
                .unwrap_or(false),
            AudioFormat::WavPack => self
                .available_tools
                .get("wavpack")
                .copied()
                .unwrap_or(false),
...
pub async fn convert_with_backend(
    ...
    if item.append_lineage {
        ...
        if settings.lineage_file_path.is_none() {
            log::debug!(
                "Lineage feature enabled but no Lineage.txt found for {}",
                input_path.display()
            );
        } else {
            log::info!(
                "Lineage feature enabled and file found: {:?}",
                settings.lineage_file_path
            );
        }
    } else {
    ...
    backend
        .convert_item_with_settings(item, input_path, output_path, progress_tx, settings)
        .await
}
