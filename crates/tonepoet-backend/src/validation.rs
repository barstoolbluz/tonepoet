...
pub fn validate_settings(settings: &ConversionSettings) -> Result<()> {
    ...
    if let Some(depth) = settings.bit_depth {
            ...
            && depth != 33
        {
            ...
            let suggestion = match depth {
                64 => {
                    "Invalid bit depth: 64. Did you mean 32 (integer) or 320 (float32)?".to_string()
                }
                _ => format!(
                    "Invalid bit depth: {}. Supported: 8, 16, 24, 32, 320 (float32)",
                    depth
                ),
            };
    ...
    if let Some(rate) = settings.sample_rate {
        if rate != 0 && (rate < 8000 || rate > 384000) {
            return Err(ConversionError::InvalidSettings(format!(
                "Invalid sample rate: {}. Range: 8000-384000 Hz",
                rate
            )));
        }
    }
    ...
    if let Some(quality) = settings.resample_quality {
        if quality > 4 {
            return Err(ConversionError::InvalidSettings(format!(
                "Invalid resample quality: {}. Range: 0-4",
                quality
            )));
        }
    }
    ...
    if let Some(level) = settings.compression_level {
        match settings.format {
            ...
            AudioFormat::WavPack => {
                if level > 6 {
                    return Err(ConversionError::InvalidSettings(format!(
                        "WavPack compression level must be 0-6, got {}",
                        level
                    )));
                }
            }
    ...
    if settings.format == AudioFormat::Mp3 {
        ...
        if let Some(bitrate) = settings.mp3_bitrate {
            if bitrate < 32 || bitrate > 320 {
                return Err(ConversionError::InvalidSettings(format!(
                    "MP3 bitrate must be 32-320 kbps, got {}",
                    bitrate
                )));
            }
        }
        ...
        if let Some(quality) = settings.mp3_quality {
            if quality > 9 {
                return Err(ConversionError::InvalidSettings(format!(
                    "MP3 VBR quality must be 0-9, got {}",
                    quality
                )));
            }
        }
    ...
    match settings.format {
        AudioFormat::Opus => {
            ...
            if let Some(depth) = settings.bit_depth {
                if depth != 0 && depth != 16 {
                    log::warn!(
                        "Opus always uses 16-bit depth internally, ignoring bit_depth={}",
                        depth
                    );
                }
            }
