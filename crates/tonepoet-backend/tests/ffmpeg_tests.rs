...
fn test_aiff_with_float_fallback() {
    ...
    let cmd = builder
        .build(Path::new("input.flac"), Path::new("output.aiff"), &settings)
        .unwrap();
...
fn test_wav_with_float() {
    ...
    let builder = CommandBuilder::new(Backend::FFmpeg);
    let cmd = builder
        .build(Path::new("input.flac"), Path::new("output.wav"), &settings)
        .unwrap();
...
fn test_flac_conversion() {
    ...
    let builder = CommandBuilder::new(Backend::FFmpeg);
    let cmd = builder
        .build(Path::new("input.wav"), Path::new("output.flac"), &settings)
        .unwrap();
...
fn test_mp3_vbr() {
    ...
    let builder = CommandBuilder::new(Backend::FFmpeg);
    let cmd = builder
        .build(Path::new("input.flac"), Path::new("output.mp3"), &settings)
        .unwrap();
