use tonepoet_true_peak::{
    ...
    ReportingPeakMeter, SearchStatus, TruePeakError, FAST_WALL_SECONDS_PER_PROGRAMME_MINUTE,
    REFERENCE_INTERVAL_OBJECTIVE_DB, STANDARD_INTERVAL_OBJECTIVE_DB,
};
...
fn public_surface_has_three_hq_tiers_with_explicit_contracts() {
    ...
    assert_eq!(FAST_WALL_SECONDS_PER_PROGRAMME_MINUTE, 1);
}
...
fn every_public_ceiling_tier_reports_hq1024v1_and_contains_the_sample_peak() {
    ...
    for tier in [PeakTier::Reference, PeakTier::Standard, PeakTier::Fast] {
        ...
        assert!(matches!(
            certificate.status,
            SearchStatus::Complete | SearchStatus::WorkLimited | SearchStatus::TimeLimited
        ));
    }
}
...
#[test]
fn fast_never_claims_work_limited_status() {
    let samples = signal(257, 1);
    let mut meter = CertifiedPeakMeter::new(192_000, 1, EdgePolicy::RepeatEndpoints, PeakTier::Fast).unwrap();
    meter.push_interleaved(&samples).unwrap();
    let certificate = meter.finalize().unwrap();
    assert!(matches!(certificate.status, SearchStatus::Complete | SearchStatus::TimeLimited));
    if certificate.status == SearchStatus::TimeLimited {
        assert!(
            certificate.diagnostics.time_limited_tiles > 0
                || certificate.diagnostics.time_bounded_prefix_blocks_skipped > 0
        );
        if certificate.diagnostics.time_limited_tiles > 0 {
            assert!(
