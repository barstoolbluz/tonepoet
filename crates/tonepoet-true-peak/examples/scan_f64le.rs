...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 5 {
        return Err("usage: scan_f64le <path> <sample-rate-hz> <channels> <reporting4x|headroom-standard|headroom64x|headroom16x|headroom8x>".into());
    }
    ...
    let mode = match args[4].as_str() {
        ...
        "headroom-standard" | "headroom-fast16x" => TruePeakMode::HeadroomStandard16x,
