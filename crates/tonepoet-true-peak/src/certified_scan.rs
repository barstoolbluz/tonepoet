...
// Fast's public target is one second of end-to-end wall time per programme
// minute. Keep a bounded portion of that target inside the meter so ordinary
// caller overhead (chunk validation, file reads, dispatch and final reporting)
// has room without turning the public target into a best-effort fiction. The
// small fixed burst pays one-time plan/allocation cost without becoming a
// duration-independent budget: on long material the 0.90 s/min slope dominates.
...
#[inline]
fn exact_magnitude(value: f64) -> f64 {
...
#[inline]
fn max_exact_magnitude(current: f64, value: f64) -> f64 {
    f64::from_bits(magnitude_bits(current).max(magnitude_bits(value)))
}

#[inline]
fn max_nonnegative_finite(left: f64, right: f64) -> f64 {
...
#[inline]
fn lower_sub(magnitude: f64, error: f64) -> f64 {
...
#[inline]
fn upper_mul(left: f64, right: f64) -> f64 {
    let Some(left_bits) = nonnegative_finite_bits(left) else {
        return f64::INFINITY;
    };
    let Some(right_bits) = nonnegative_finite_bits(right) else {
...
fn dot_rounding_upper(operations: usize, absolute_sum_upper: f64) -> f64 {
    upper_add(
...
impl CertifiedTwoXEngine {
    fn new(spec: ReconstructionSpec, policy: SearchPolicy, channels: usize) -> Self {
        let inner = match policy {
            ...
            SearchPolicy::Fast90 | SearchPolicy::Fast1sPerMinute => {
    ...
    fn process_frame(&mut self, frame: &[f64], input_index: i128, scanner: &mut CertifiedScanner) {
        ...
        let execute_fft = scanner.policy != SearchPolicy::Fast1sPerMinute
...
impl CertifiedScanner {
    ...
    fn begin_processing_at(&mut self, started: Instant) {
        if self.policy == SearchPolicy::Fast1sPerMinute && self.processing_started.is_none() {
    ...
    fn fast_time_budget_exhausted(&self) -> bool {
        self.policy == SearchPolicy::Fast1sPerMinute
    ...
    fn retain_fast_tile_fallback_channel(
        ...
        debug_assert_eq!(self.policy, SearchPolicy::Fast1sPerMinute);
    ...
    fn process_global_heap(
        ...
        let mut available = (0..self.channels)
            .map(|channel| match self.policy {
                SearchPolicy::Reference9 | SearchPolicy::Fast1sPerMinute => u64::MAX,
        ...
        while let Some(node) = heap.pop() {
            ...
            if self.policy == SearchPolicy::Fast1sPerMinute && self.fast_time_budget_exhausted() {
                time_limited = true;
        ...
        let fast90_magnitude_summaries = match self.policy {
            ...
            SearchPolicy::Fast90 | SearchPolicy::Fast1sPerMinute => {
                ...
                for channel in 0..self.channels {
                    ...
                    if self.policy == SearchPolicy::Fast1sPerMinute
    ...
    fn finalize(
        ...
        let status = match self.policy {
            ...
            SearchPolicy::Fast1sPerMinute => SearchStatus::TimeLimited,
        };
...
impl CertifiedPeakMeterImpl {
    pub(super) fn new(
        ...
        let construction_started =
            (policy == SearchPolicy::Fast1sPerMinute).then(Instant::now);
    ...
    pub(super) fn push_interleaved(&mut self, samples: &[f64]) -> Result<(), TruePeakError> {
        let validation_started =
            (self.scanner.policy == SearchPolicy::Fast1sPerMinute).then(Instant::now);
