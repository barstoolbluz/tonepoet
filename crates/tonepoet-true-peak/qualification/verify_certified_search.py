...
FAST_RUST = ROOT / "src" / "fast_scan.rs"
...
def audit() -> dict:
    ...
    fast90_prefix_executor_is_policy_scoped = (
        ...
        and "SearchPolicy::Fast90 | SearchPolicy::RetiredClockFast1s" in scanner_source
    ...
    binding_benchmark_uses_total_timer_without_projection = (
        ...
        and "let total_elapsed = total_started.elapsed();" in bench_source
        and "total_elapsed.as_nanos() <= target_nanos" in bench_source
    ...
    benchmark_exposes_only_three_tiers = (
        '"reference" => (PeakTier::Reference, "not-fast")' in bench_source
        and '"standard" => (PeakTier::Standard, "not-fast")' in bench_source
        and '"fast" => (PeakTier::Fast, "production")' in bench_source
        and "legacy-reference" not in bench_source
        and "fast_wall_target_met" in bench_source
        and "FastCommissioningMode" in bench_source
        and '#[cfg(feature = "fast-stage-timing")]' in bench_source
    )
